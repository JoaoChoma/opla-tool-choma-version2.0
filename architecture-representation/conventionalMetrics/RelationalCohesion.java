...
import br.uem.din.architectureEvolution.representation.Attribute;
...
import br.uem.din.architectureEvolution.representation.Method;
	...
	private int searchOperationClassDependencies (Class source, Component comp){
 		...
 		for (Class c:  classes) {
			...
			for (Method method: allOperations){
				...
				for (String parameter: parameters) {
					if ((parameter.contains(c.getName())) && (!(operationDepClasses.contains(c)))) operationDepClasses.add(c);
				}
			}
		}
		...
		for (Interface itf: interfaces){
			...
			for (Method method: allOperations){
				Collection<String> parameters = method.getParameters().values();
				for (String parameter: parameters) {
					if ((parameter.contains(itf.getName())) && (!(operationDepInterfaces.contains(itf)))) operationDepInterfaces.add(itf);
				}
			}
		}
...
private int searchAssociationClassDependencies (Class source, Component comp){
	...
	for (Class c: comp.getClasses()){
		...
		for (InterClassRelationship relationship : relationships) {
			if (relationship instanceof GeneralizationInterClassRelationship){
				...
				if (generalization.getParent().equals(c.getName()) && (!(associationDepClasses.contains(c)))) associationDepClasses.add(c);
			...
			if (relationship instanceof DependencyInterClassRelationship){
				...
				if (dependency.getClient().equals(c.getName()) && (!(associationDepClasses.contains(c)))) associationDepClasses.add(c);
			}
	...
	for (Interface itf: comp.getImplementedInterfaces()){
		...
		for (InterClassRelationship relationship : relationships) {
			if (relationship instanceof GeneralizationInterClassRelationship){
				GeneralizationInterClassRelationship generalization = (GeneralizationInterClassRelationship) relationship;
				if (generalization.getParent().equals(itf.getName()) && (!(associationDepInterfaces.contains(itf)))) associationDepInterfaces.add(itf);
					
			}
			
			if (relationship instanceof DependencyInterClassRelationship){
				DependencyInterClassRelationship dependency = (DependencyInterClassRelationship) relationship;
				if (dependency.getClient().equals(itf.getName()) && (!(associationDepInterfaces.contains(itf)))) associationDepInterfaces.add(itf);
			}

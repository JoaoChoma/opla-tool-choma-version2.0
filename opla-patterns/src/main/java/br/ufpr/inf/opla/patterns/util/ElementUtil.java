...
public class ElementUtil {
	...
	public static List<Interface> getAllCommonInterfaces(List<Element> participants) {
		...
		for (Element participant : participants) {
			...
			if (first) {
				interfaces.addAll(
						CollectionUtils.union(elementInterfaces, AdapterUtil.getAllTargetInterfaces(participant)));
				if (participant instanceof Interface) {
					interfaces.add((Interface) participant);
				}
				first = false;
			} else {
				elementInterfaces = new ArrayList<>(
						CollectionUtils.union(elementInterfaces, AdapterUtil.getAllTargetInterfaces(participant)));
				if (participant instanceof Interface) {
	...
	public static Set<Concern> getOwnAndMethodsCommonConcerns(List<Element> elements) {
		...
		for (Element participant : elements) {
			commonConcerns = new HashSet<>(
					CollectionUtils.intersection(commonConcerns, getOwnAndMethodsConcerns(participant)));
		}
	...
	public static Set<Concern> getOwnAndMethodsCommonConcernsOfAtLeastTwoElements(List<Element> elements) {
		...
		for (Element iElement : elements) {
			concernLoop: for (Concern concern : getOwnAndMethodsConcerns(iElement)) {
	...
	public static HashMap<Concern, List<Element>> groupElementsByConcern(List<Element> elements) {
		...
		ArrayList<Element> nullList = ElementUtil
				.getElementsWithNoOwnConcernsAndWithAtLeastOneMethodWithNoConcerns(elements);
		if (!nullList.isEmpty()) {
	...
	public static ArrayList<Element> getElementsWithNoOwnConcernsAndWithAtLeastOneMethodWithNoConcerns(
			Iterable<Element> elements) {
		ArrayList<Element> nullArrayList = new ArrayList<>();
		elementLoop: for (Element element : elements) {
	...
	public static void implementInterface(List<Element> elements, Interface anInterface, List<Element> adapterList,
			List<Element> adapteeList) {
	...
	public static arquitetura.representation.Class implementInterface(Element child, Interface anInterface) {
		if (!ElementUtil.isTypeOf(child, anInterface)
				&& !AdapterUtil.getAllTargetInterfaces(child).contains(anInterface)) {
	...
	public static List<Element> getChainOfRelatedElementsWithSameConcern(List<Element> mainElements, Concern concern) {
		...
		for (Element element : mainElements) {
			if ((element instanceof Class || element instanceof Interface) && !elements.contains(element)) {
	...
	private static void getChainOfRelatedElementsWithSameConcern(Element element, Concern concern,
			List<Element> elements, List<Element> mainElements) {
		...
		for (Relationship relationship : relationships) {
			...
			//	System.out.println("chained null");
			//}
			...
			if (chainedElement == null) {
				...
				if (chainedElement == null) {
					continue;
				} else if (element.equals(chainedElement)) {
			...
			if (chainedElement != null && (chainedElement instanceof Class || chainedElement instanceof Interface)
					&& !chainedElement.equals(element) && chainedElement.getAllConcerns().contains(concern)
					&& !elements.contains(chainedElement) && mainElements.contains(chainedElement)) {

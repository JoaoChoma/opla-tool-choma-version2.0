		...
		for (int i = 0; i < list.size(); i++) {
			for (int j = 0; j < numberOfVariables; j++) {
				...
				if (objectiveFuncs.contains("wocsc"))
					allMetrics.getWocsclass()
							.add(buildWocsclassMetrics(idSolution, execution, experiement, metrics, arch));
				
				if (objectiveFuncs.contains("wocsi"))
					allMetrics.getWocsinterface()
							.add(buildWocsinterfaceMetrics(idSolution, execution, experiement, metrics, arch));
				
				if (objectiveFuncs.contains("cbcs"))
					allMetrics.getCbcs()
							.add(buildCbcsMetrics(idSolution, execution, experiement, metrics, arch));
				
				if (objectiveFuncs.contains("ssc"))
					allMetrics.getSsc()
							.add(buildSscMetrics(idSolution, execution, experiement, metrics, arch));
				
				
				if (objectiveFuncs.contains("svc"))
					allMetrics.getSvc()
							.add(buildSvcMetrics(idSolution, execution, experiement, metrics, arch));
				
				if (objectiveFuncs.contains("av"))
					allMetrics.getAv()
							.add(buildAvMetrics(idSolution, execution, experiement, metrics, arch));

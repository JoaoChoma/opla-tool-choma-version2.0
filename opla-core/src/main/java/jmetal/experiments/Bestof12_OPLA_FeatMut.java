...
import jmetal.metaheuristics.memetic.Bestof12;
...
public class Bestof12_OPLA_FeatMut {
	...
	public Bestof12_OPLA_FeatMut(NSGAIIConfig config) {
		this.configs = config;
	}

	public Bestof12_OPLA_FeatMut() {
	}
	...
	public void execute() throws FileNotFoundException, IOException, JMException, ClassNotFoundException {
		...
		for (String pla : plas) {
			...
			Experiment experiement = mp.createExperimentOnDb(plaName, "Bestof12", configs.getDescription());
			ExperimentConfs conf = new ExperimentConfs(experiement.getId(), "Bestof12", configs);
			conf.save();
			...
			algorithm = new Bestof12(problem);

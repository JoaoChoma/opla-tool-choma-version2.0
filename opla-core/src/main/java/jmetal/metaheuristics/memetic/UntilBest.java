...
public class UntilBest extends Algorithm {
	...
	public void criarArquivo() throws IOException {
		File f = new File("selecao-UntilBest.txt");
		f.createNewFile();
		File t = new File("tempo-UntilBest.txt");
		t.createNewFile();
		File a = new File("aplicacao-UntilBest.txt");
		a.createNewFile();
	}

	public void gravarTempo(long valor) throws IOException {
		FileWriter t = new FileWriter("tempo-UntilBest.txt", true);
		t.write("\n tempo-UntilBest: " + valor);
		t.close();
	}
...
//
	public void gravarAplicacao(String input) throws IOException {
		FileWriter t = new FileWriter("aplicacao-UntilBest.txt", true);
		t.write("\n " + input);
		t.close();
	}
			...
			while (evaluations < maxEvaluations) {
				...
				for (int i = 0; i < (populationSize / 2); i++) {
					if (evaluations < maxEvaluations) {
						...
						System.out.println("\n --- \n");
						System.out.println("UntilBest INDIVIDUO: " + i + " evolucao: " + evaluations + " \n");
						System.out.println("\n ---\n ");
						// tempo
						...
						Local_Search_Until local0 = new Local_Search_Until(offSpring[0], offSpringForLocal[0], problem_, operatorLocal);
						Local_Search_Until local1 = new Local_Search_Until(offSpring[1], offSpringForLocal[1], problem_, operatorLocal);
						...
						if (problem_.isPatterns(offSpringForLocal[0])) {
							this.gravarAplicacao("solucao COM patterns");
						} else {
							this.gravarAplicacao("solucao SEM patterns");
						}
